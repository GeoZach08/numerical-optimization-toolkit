...
fprintf('--- Εκτέλεση Θέματος 2 (Μέθοδος Χρυσού Τομέα) ---\n');
...
for i = 1:length(functions) 
    for j = 1:length(l_vector) 
        ...
        [~, ~, ~, evals] = golden_section(current_f, a_init, b_init, current_l);
        eval_results_l(i, j) = evals;
    end
end

figure('Name', 'Μελέτη 1 (Θέμα 2): Υπολογισμοί vs l');
hold on;
...
hold off;
title('Υπολογισμοί f(x) συναρτήσει του l (Μέθοδος Χρυσού Τομέα)');
...
for i = 1:length(functions) 
    ...
    current_f = functions{i};
    figure('Name', ['Μελέτη 2 (Θέμα 2): Σύγκλιση [a_k, b_k] για ' function_names{i}]);
    hold on;
    ...
    for j = 1:length(l_demo_values)
        ...
        [~, ~, iters, ~, a_hist, b_hist] = ...
            golden_section(current_f, a_init, b_init, current_l);
    ...
    hold off;
    title(['Σύγκλιση [a_k, b_k] για ' function_names{i} ' (Χρυσός Τομέας)']);
...
fprintf('--- Ολοκληρώθηκε το Θέμα 2 ---\n');