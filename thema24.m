...
for m = 1:3
    ...
    figure('Name', ['LM - ' method_titles{m}]);
    contourf(X, Y, Z, 20);
    colorbar; colormap jet; hold on;
    title(['Levenberg-Marquardt: ' method_titles{m}]);
    xlabel('x'); ylabel('y');
    
    for p = 1:size(start_points, 1)
        x0 = start_points(p, :)'; 
       
        [x_opt, f_opt, iter, path] = levenberg_marquardt(f, grad_f, hess_f, x0, epsilon, current_method);
...
figure('Name', 'LM: Ταχύτητα Σύγκλισης (Start: -1, -1)');
x0_compare = [-1; -1];
styles = {'r-o', 'g-s', 'b-d'};
colors_plot = {'r', 'g', 'b'}; 
hold on;
...
for m = 1:3
    [~, ~, ~, path] = levenberg_marquardt(f, grad_f, hess_f, x0_compare, epsilon, step_methods{m});
...
title('Σύγκλιση LM (Τιμή f(x), Start: -1, -1)');
